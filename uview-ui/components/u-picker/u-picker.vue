	...
	<u-popup :maskCloseAble="maskCloseAble" mode="bottom" :popup="false" v-model="value" length="auto"
		...
		<view class="u-datetime-picker">
			...
			<view class="u-picker-body">
				...
				<picker-view v-else-if="mode == 'multiSelector'" :value="valueArr" @change="change"
					class="u-picker-view" @pickstart="pickstart" @pickend="pickend">
...
<script>
	...
	export default {
		...
		watch: {
			...
			yearAndMonth(val) {
				if (this.params.year || this.params.month) this.setDays();
			},
			yearAnd(val) {
		...
		methods: {
			...
			setYears() {
				...
				let endy = this.endYear
				if (this.startTime) {
					starty = new Date(this.startTime).getFullYear()
				}
				if (this.endTime) {
					endy = new Date(this.endTime).getFullYear()
				}
				this.years = this.generateArray(starty, endy);
			...
			setMonths() {
				...
				let minM = 1,
					maxM = 12
				if (this.startTime) {
					let y = new Date(this.startTime).getFullYear()
					if (this.year == y) {
						minM = new Date(this.startTime).getMonth() + 1
					}
				}
				if (this.endTime) {
					let y = new Date(this.endTime).getFullYear()
					if (this.year == y) {
						maxM = new Date(this.endTime).getMonth() + 1
					}
				}
				this.months = this.generateArray(minM, maxM);
				this.valueArr.splice(this.valueArr.length - 1, 1, this.getIndex(this.months, this.month));
			},
			setDays() {
				...
				if (this.startTime) {
					let y = new Date(this.startTime).getFullYear()
					let m = new Date(this.startTime).getMonth()
					if (this.year == y && this.month == m + 1) {
						minD = new Date(this.startTime).getDate()
					}
				}
				if (this.endTime) {
					let y = new Date(this.endTime).getFullYear()
					let m = new Date(this.endTime).getMonth()
					if (this.year == y && this.month == m + 1) {
						totalDays = new Date(this.endTime).getDate()
					}
				}
				this.days = this.generateArray(minD, totalDays);
				let index = 0;
